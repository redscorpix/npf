...
npf.string.phoneNumber.parse = function(phone) {
	...
	var codeLength;
	var one = [0, 1, 3, 7];
	var two = [20, 27, 30, 31, 32, 33, 34, 36, 40, 41, 43, 44, 45, 46, 47, 48, 49, 51, 52, 53, 54, 55, 56, 57, 58, 60, 61, 62, 63, 64, 65, 66, 81, 82, 84, 86, 90, 91, 92, 93, 94, 95, 98];
	...
	if (4 == phone.length) {
		...
		result += phone.substr(0, 1) + '-' + phone.substr(1, 2) + '-' + phone.substr(3);
	} else if (6 == phone.length) {
		result += phone.substr(0, 2) + '-' + phone.substr(2, 2) + '-' + phone.substr(4);
	}
