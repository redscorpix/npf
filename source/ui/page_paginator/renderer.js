...
/**
 * @param {npf.ui.PagePaginator} block
 * @return {!Element}
 */
npf.ui.pagePaginator.Renderer.prototype.createDom = function(block) {
	/** @type {!Element} */
	...
	/** @type {Element} */
	var indicatorsElement = this.createPageIndicatorsElement(block);
 ...
 * @return {Element}
 */
npf.ui.pagePaginator.Renderer.prototype.createPageIndicatorsElement = function(block) {
	/** @type {Element} */
	var element = goog.dom.createDom(goog.dom.TagName.INS, this.getPageIndicatorsCssClass());
	/** @type {number} */
	var pageCount = block.getPageCount();

	for (var i = 0; i < pageCount; i++) {
		/** @type {Element} */
		var subElement = goog.dom.createDom(goog.dom.TagName.INS, this.getPageIndicatorCssClass());
 ...
 */
npf.ui.pagePaginator.Renderer.prototype.setPrevEnabled = function(block, enable) {
	/** @type {Element} */
	var prevElement = block.getPrevElement();

	if (prevElement) {
		goog.dom.classes.enable(prevElement, this.getPrevDisabledCssClass(), !enable);
	}
};
 ...
 * @param {boolean} enable
 */
npf.ui.pagePaginator.Renderer.prototype.setNextEnabled = function(block, enable) {
	/** @type {Element} */
	var nextElement = block.getNextElement();

	if (nextElement) {
		goog.dom.classes.enable(nextElement, this.getNextDisabledCssClass(), !enable);
	}
};
 ...
 */
npf.ui.pagePaginator.Renderer.prototype.setSelected = function(block, index, select) {
	/** @type {Element} */
	...
	if (element) {
		...
		if (indicatorElements[index]) {
			goog.dom.classes.enable(indicatorElements[index], this.getSelectedPageIndicatorCssClass(), select);
		}
	}
};
