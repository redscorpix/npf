...
npf.style.animation.setPlayState = function(element, name, opt_playState) {
  ...
  if (-1 < index) {
    /** @type {!Array.<string>} */
    var playStates = goog.array.map(animations, function(animation) {
...
npf.style.animation.setAnimations_ = function(element, animations) {
  ...
  var playStates = [];
  /** @type {!Array.<string>} */
  var values = goog.array.map(animations, function(animation) {
...
npf.style.animation.getAnimations = function(element) {
  ...
  return goog.array.map(names, function(name, i) {
    return {
      name: name,
