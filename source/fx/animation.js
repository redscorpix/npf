 ...
 * @param {Array.<number>|npf.fx.Animation.Timing|function(number):number|null=} opt_acc Acceleration function, returns 0-1 for inputs 0-1.
 * @constructor
...
npf.fx.Animation = function(start, end, duration, opt_acc) {
	...
	goog.base(this, start, end, duration, /** @type {Function|null|undefined} */ (accel));
};
...
npf.fx.Animation.getAcceleration = function(accel, duration) {
	return function(t) {
		return npf.fx.CubicBezier(t, accel[0], accel[1], accel[2], accel[3], duration);
	};
};
...
npf.fx.Animation.prototype.cycle = function(now) {
	/** @type {?number} */
	var time = npf.fx.Animation.enabled || now == this.startTime ? now: this.endTime;
